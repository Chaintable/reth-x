...
use revm::{
    database::{BundleState, InMemoryDB, State},
    primitives::HashMap,
...
/// Helper alias type for the state's [`StateDiffTraceDB`] wrapping a [`StateCacheDb`].
pub type StateDiffDb = StateDiffTraceDB<StateCacheDb>;
...
///
/// Reads are delegated to the underlying database, writes are committed to both the underlying
/// database and the in-memory diff. After execution, [`Self::diff`] contains all the changes made
/// during execution and can be compared against a pre-state database to compute a state diff.
#[derive(Debug, Clone)]
pub struct StateDiffTraceDB<ExtDB> {
    /// The diff that stores all state changes.
    pub diff: InMemoryDB,
    /// The underlying database used to load data and execute against.
    pub db: ExtDB,
}

impl<ExtDB> StateDiffTraceDB<ExtDB> {
    /// Creates a new [`StateDiffTraceDB`] wrapping the given database.
